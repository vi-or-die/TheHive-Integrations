...
# TO BEGIN....
# 
# Configure the script to run properly the following items need to be completed
...
$MSSCert = Get-ChildItem -Path Cert:\LocalMachine\My | where-Object {$_.Subject -like 'O=Company Inc, CN=SII123456, L=City, S=State, C=Country'}

# 3. Update the following items with the correct URI for TheHive API, and your API key.
[string]$HiveUri = "http://hostname.company.com:9002/api"
[string]$HiveToken = "TheHive_API_Key_Goes_Here"

# 4. Add usernames to the Whitelist array to remove unwanted items. 
$Whitelist = "common.service","system"
############ Define Functions #############
...
param(
    ...
    $SanatizedDescription = $Description -replace '“', '"'
    ...
    $Artifact = "" | select dataType, data, message, tags
        ...
        foreach ($Signature in $Alert_Data.SignatureList.Signature){
            ...
            foreach ($user in $UserNames){
                # Weed out Hostnames from Usernames
                if ($user -match '$'){
                    $HostNames += $user
                }
                else{
            ...
            # Exctract info from Correlated events
            if ($Signature.CorrelatedEvent -match "Yes"){
                $FileList = $Signature.CorrelatedEventList.event.FileDetails.File
                foreach ($file in $FileList){
                    #if ($file -match "[(.:\\\s)|(.:\s)]") {continue}
                    ...
                    # URL
                        ...
                        $Artifacts += $Artifact
                    # FileNames
                        # Removes any orphaned drive letters or filenames
                        if ($FileName -match "([A-z]:\\((\w)|([!@#$%^&*()_\-+={}\[\]\|\\:;<,>.?/])))"){
                            $Artifact = "" | select dataType, data, message, tags
        ...
        $Condensedresult = foreach ($value in $Condensed){
            if (!($Whitelist.Contains($value.Name))){
